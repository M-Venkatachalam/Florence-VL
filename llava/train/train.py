...
class ModelArguments:
    ...
    tune_vision_model: bool = field(default=False)
...
def get_vision_tower_state_maybe_zero_3(named_params, keys_to_match=['']):
    to_return = {k: t for k, t in named_params if any(key_match in k for key_match in keys_to_match)}
    to_return = {k: maybe_zero_3(v, ignore_status=True).cpu()for k, v in to_return.items()}
    # to_return = {k.replace('vision_tower.', '', 1): maybe_zero_3(v, ignore_status=True).cpu() for k, v in to_return.items()}
    # to_return = {'model.vision_tower.' + k: maybe_zero_3(v, ignore_status=True).cpu() for k, v in to_return.items()}
    return to_return
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    if getattr(trainer.args, "tune_entire_model", False):
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            if 'docmatix' in image_file:
                image = Image.open(read_bin_as_bytesio(os.path.join(image_folder, image_file))).convert('RGB')
            else:
            ...
            if self.data_args.image_aspect_ratio == 'pad':
                ...
                image = processor(text=task, images=image, return_tensors='pt', image_mean=processor.image_mean, image_std=processor.image_std, padding=True)['pixel_values'][0]
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        if 'mpt' in model_args.model_name_or_path:
            ...
            print("loading the checkpoint {}".format(model_args.model_name_or_path))
